...
func (r *Repository) CreateAdmin(ctx context.Context, a *domain.Admin) error {
	...
	if err != nil {
		if postgres.IsUniqueViolation(err, "admins_email_key") {
...
func (r *Repository) DeleteAdmin(ctx context.Context, adminID uuid.UUID) error {
	tag, err := r.db.Pool.Exec(ctx, `DELETE FROM admins WHERE id = $1`, adminID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
...
func (r *Repository) CountAdmins(ctx context.Context) (int, error) {
	var n int
	err := r.db.Pool.QueryRow(ctx, `SELECT count(*) FROM admins WHERE is_active`).Scan(&n)
	return n, err
}
...
func (r *Repository) FindSession(ctx context.Context, token string) (*SessionWithAdmin, error) {
	...
	err := r.db.Pool.QueryRow(ctx, query, hashToken(token)).Scan(
